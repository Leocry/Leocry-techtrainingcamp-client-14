...
class ShapesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    DateTime now = DateTime.now().add(Duration(hours: int.parse(offset)-8));
    final paint = Paint();
    ...
    // x = cx + r * cos(a)
    x = (size.width / 2) + (size.width / 3 - 25) * cos(Vector.radians(minuteDegree));
    // y = cy + r * sin(a)
    y = (size.height / 2) + (size.width / 3 - 25) * sin(Vector.radians(minuteDegree));
    ...
    if(h>12)
      h-=12;
    double hourseDegree = 360 / 12 * h;
    hourseDegree += 30 / 60 * now.minute;
    //print(h);
    // x = cx + r * cos(a)
    x = (size.width / 2) + (size.width / 3 - 35) * cos(Vector.radians(hourseDegree));
    // y = cy + r * sin(a)
    y = (size.height / 2) + (size.width / 3 - 35) * sin(Vector.radians(hourseDegree));
